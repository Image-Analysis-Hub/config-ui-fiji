package org.scijava.ui.config.fiji.roi;
...
public class Polygon2D implements Iterable< Point2D >
{
	 ...
	 */
	public static final Polygon2D create( final List< Point2D > vertices )
	{
	 ...
	 * @return a new polygon
	 */
	public static final Polygon2D create( final double[] xdata, final double[] ydata )
	{
	 ...
	 */
	public Polygon2D( final double[] xCoords, final double[] yCoords )
	{
	 ...
	 */
	public Polygon2D( final Collection< Point2D > vertices )
	{
	...
	public Box2D boundingBox()
	{
		...
		for ( final Point2D vertex : this.vertices )
		{
	...
	public double signedArea()
	{
		...
		for ( int i = 0; i < n; i++ )
		{
			final Point2D p1 = this.vertices.get( i );
			final Point2D p2 = this.vertices.get( ( i + 1 ) % n );
	...
	public Point2D centroid()
	{
		...
		for ( int i = 1; i <= n; i++ )
		{
			final Point2D p1 = this.vertices.get( i - 1 );
			final Point2D p2 = this.vertices.get( i % n );
			final double x1 = p1.getX();
			final double y1 = p1.getY();
			final double x2 = p2.getX();
			final double y2 = p2.getY();
			final double common = x1 * y2 - x2 * y1;
	...
	public boolean contains( final Point2D point )
	{
		...
		for ( final Point2D current : vertices )
		{
	 ...
	 * Returns:
	 * <ul>
	 * <li>positive value for query point left of the line through P0 and
	 * P1</li>
	 ...
	 */
	private final static int isLeft( final Point2D p1, final Point2D p2, final Point2D pt )
	{
	...
	public Polygon2D invert()
	{
		...
		final Polygon2D result = new Polygon2D( n );
	...
	public PolygonRoi createRoi()
	{
		...
		for ( int i = 0; i < n; i++ )
		{
			final Point2D p = this.vertices.get( i );
			px[ i ] = ( float ) p.getX();
			py[ i ] = ( float ) p.getY();
		}
	 ...
	 */
	public int addVertex( final Point2D position )
	{
		final int n = this.vertices.size();
	 ...
	 *            the position of the new vertex
	 */
	public void setVertex( final int i, final Point2D pos )
	{
